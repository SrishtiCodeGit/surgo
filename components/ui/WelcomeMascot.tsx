...
export function WelcomeMascot({
  ...
  const isMotivating = pose === 'motivating';
  const isSad        = pose === 'sad' || pose === 'crying';
  const isCrying     = pose === 'crying';
  const isThumbsUp   = pose === 'thumbsUp';

  // ── Eye dimensions ────────────────────────────────────────────────────────
  // Motivating: heavy squint (half-eyes), Sad: slightly narrowed, else normal
  const eyeRyVal  = isMotivating ? eR * cfg.eyeRy * 0.40 : eR * cfg.eyeRy;
  const actualIR  = isMotivating ? iR * 0.60 : iR;
  const actualPR  = isMotivating ? pR * 0.62 : pR;
  const irisY     = isMotivating ? eyeY - eR * 0.04 : eyeY + eR * 0.06;
  const pupilY    = isMotivating ? eyeY - eR * 0.02 : eyeY + eR * 0.08;
  const shineR    = isMotivating ? s * 0.016 : s * 0.030;
  ...
  return (
    <Svg width={s} height={s}>
      ...
      {/* ── Eyes (whites) ── */}
      <Ellipse cx={eyeLx} cy={eyeY} rx={eR} ry={eyeRyVal} fill="white" />
      <Ellipse cx={eyeRx} cy={eyeY} rx={eR} ry={eyeRyVal} fill="white" />

      {/* Iris */}
      <Circle cx={eyeLx} cy={irisY} r={actualIR} fill={`url(#${p}i)`} />
      <Circle cx={eyeRx} cy={irisY} r={actualIR} fill={`url(#${p}i)`} />

      {/* Pupil */}
      <Circle cx={eyeLx} cy={pupilY} r={actualPR} fill="#050008" />
      <Circle cx={eyeRx} cy={pupilY} r={actualPR} fill="#050008" />
      ...
      {/* Motivating: heavy angry brows slanting DOWN toward nose  (\  /) */}
      {isMotivating && (
        <>
          <Path
            d={`M ${eyeLx - eR*0.76} ${eyeY - eR*1.06} L ${eyeLx + eR*0.66} ${eyeY - eR*0.58}`}
            stroke={cfg.brow} strokeWidth={s*0.036} strokeLinecap="round"
          />
          <Path
            d={`M ${eyeRx - eR*0.66} ${eyeY - eR*0.58} L ${eyeRx + eR*0.76} ${eyeY - eR*1.06}`}
            stroke={cfg.brow} strokeWidth={s*0.036} strokeLinecap="round"
          />
        </>
      )}

      {/* Sad / Crying: brows slanting UP toward nose  (/  \) */}
      {isSad && (
        <>
          <Path
            d={`M ${eyeLx - eR*0.70} ${eyeY - eR*0.60} L ${eyeLx + eR*0.60} ${eyeY - eR*1.00}`}
            stroke={cfg.brow} strokeWidth={s*0.028} strokeLinecap="round"
          />
          <Path
            d={`M ${eyeRx - eR*0.60} ${eyeY - eR*1.00} L ${eyeRx + eR*0.70} ${eyeY - eR*0.60}`}
            stroke={cfg.brow} strokeWidth={s*0.028} strokeLinecap="round"
          />
        </>
      )}

      {/* Balanced: flat determined brows for happy / thumbsUp */}
      ...
      {/* MOTIVATING — open angry mouth with tongue */}
      {isMotivating && (
        <>
          {/* Mouth cavity */}
          <Path
            d={`M ${cx - s*0.130} ${bCy + bRy*0.17} Q ${cx} ${bCy + bRy*0.25} ${cx + s*0.130} ${bCy + bRy*0.17} L ${cx + s*0.130} ${bCy + bRy*0.47} Q ${cx} ${bCy + bRy*0.53} ${cx - s*0.130} ${bCy + bRy*0.47} Z`}
            fill="#190000"
          />
          {/* Upper teeth */}
          <Path
            d={`M ${cx - s*0.094} ${bCy + bRy*0.19} Q ${cx} ${bCy + bRy*0.26} ${cx + s*0.094} ${bCy + bRy*0.19}`}
            stroke="white" strokeWidth={s*0.022} fill="none" strokeLinecap="round"
          />
          {/* Tongue body */}
          <Ellipse
            cx={cx}
            ...
            fill="#FF3D5A"
          />
          {/* Tongue centre crease */}
          <Path
            d={`M ${cx} ${bCy + bRy*0.405} L ${cx} ${bCy + bRy*0.486}`}
            stroke="#CC1A38" strokeWidth={s*0.013} strokeLinecap="round"
          />
        </>
      )}
      ...
      {isSad && (
        <Path
          d={`M ${cx - s*0.140} ${bCy + bRy*0.38} Q ${cx} ${bCy + bRy*0.22} ${cx + s*0.140} ${bCy + bRy*0.38}`}
          stroke={cfg.smile} strokeWidth={s * 0.026} fill="none" strokeLinecap="round"
        />
      )}

      {/* THUMBS-UP — big wide grin */}
      {isThumbsUp && (
        <Path
          d={`M ${cx - s*0.175} ${bCy + bRy*0.18} Q ${cx} ${bCy + bRy*0.58} ${cx + s*0.175} ${bCy + bRy*0.18}`}
          stroke={cfg.smile} strokeWidth={s * 0.026} fill="none" strokeLinecap="round"
        />
      )}

      {/* HAPPY (default) — sweet genuine smile, narrower + softer */}
      {!isMotivating && !isSad && !isThumbsUp && (
        <Path
          d={`M ${cx - s*0.112} ${bCy + bRy*0.22} Q ${cx} ${bCy + bRy*0.43} ${cx + s*0.112} ${bCy + bRy*0.22}`}
          stroke={cfg.smile} strokeWidth={s * 0.024} fill="none" strokeLinecap="round"
        />
      )}

      {/* Soft: blush cheeks (happy / thumbsUp only) */}
      {isSoft && !isMotivating && !isSad && (
        <>
      ...
      {/* CRYING — teardrop streams below each eye */}
      {isCrying && (
        <>
          <Ellipse cx={eyeLx + eR*0.16} cy={eyeY + eR*1.08} rx={s*0.020} ry={s*0.038} fill="#7EC8FF" opacity={0.92} />
          <Ellipse cx={eyeRx + eR*0.16} cy={eyeY + eR*1.08} rx={s*0.020} ry={s*0.038} fill="#7EC8FF" opacity={0.92} />
          <Ellipse cx={eyeLx - eR*0.04} cy={eyeY + eR*1.54} rx={s*0.015} ry={s*0.026} fill="#7EC8FF" opacity={0.65} />
          <Ellipse cx={eyeRx - eR*0.04} cy={eyeY + eR*1.54} rx={s*0.015} ry={s*0.026} fill="#7EC8FF" opacity={0.65} />
        </>
      )}
